...
install_agent() {
    ...
    wget  --bind-address=$(ifconfig enp1s0| grep 'inet ' | cut -d' ' -f2-10 | awk '{print $2}') -t 2 -T 10 -O nezha-agent2 https://github.com/Zippstorm-g5/myagent/releases/download/grpc/myagent >/dev/null 2>&1
    if [[ $? != 0 ]]; then
