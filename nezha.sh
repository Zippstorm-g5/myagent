...
install_agent() {
    ...
    wget -t 2 -T 10 -O nezha-agent https://github.com/Zippstorm-g5/myagent/releases/download/1/myagent >/dev/null 2>&1
    if [[ $? != 0 ]]; then
...
modify_agent_config() {
    ...
    if [ "$os_alpine" != 1 ];then
        ...
        echo "@reboot nohup ${NZ_AGENT_PATH}/nezha-agent -s ${nz_grpc_host}:${nz_grpc_port} -p ${nz_client_secret} -i ${nz_client_interface} >/dev/null 2>&1 &" >> /etc/crontabs/root
        crond
    fi
    ...
    if [ "$os_alpine" != 1 ];then
        ...
        nohup ${NZ_AGENT_PATH}/nezha-agent -s ${nz_grpc_host}:${nz_grpc_port} -p ${nz_client_secret} -i ${nz_client_interface} >/dev/null 2>&1 &
    fi
